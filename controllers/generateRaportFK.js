...
const generateHistoryDocuments = async (company) => {
  try {
    ...
    logEvents(
      `generateRaportFK, generateHistoryDocuments: ${error}`,
      "reqServerErrors.txt"
    );
  }
};
...
const getAccountancyDataMsSQL = async (company, res) => {
  try {
    ...
    //nowe zoptymalizowane zapytanie KRT
    const queryKRT = `
...
`;
    ...
    logEvents(
      `generateRaportFK, getAccountancyDataMsSQL: ${error}`,
      "reqServerErrors.txt"
    );
    return null;
  }
...
const saveAccountancyData = async (data, company) => {
  try {
    ...
    logEvents(
      `generateRaportFK, saveAccountancyData: ${error}`,
      "reqServerErrors.txt"
    );
  }
};
...
const differencesAS_FK = async (company) => {
  try {
    ...
    logEvents(
      `generateRaportFK, differencesAS_FK: ${error}`,
      "reqServerErrors.txt"
    );
    return [];
  }
