...
const newConfirmPass = async (req, res) => {
  ...
  try {
    const [checkMail] = await connect_SQL.query(
    ...
    await connect_SQL.query(
      "DELETE FROM company_password_resets WHERE EMAIL = ?",
      [userlogin]
    );
    ...
    await connect_SQL.query(
      `INSERT INTO company_password_resets (EMAIL, TOKEN) VALUES (?, ?)`,
      [userlogin, token]
    );
...
const verifyPass = async (req, res) => {
  ...
  try {
    ...
    const [verifyAccess] = await connect_SQL.query(
      `SELECT * FROM company_password_resets WHERE TOKEN = ?`,
      [token]
    );
    ...
    logEvents(
      `resetPasswordController, verifyPass: ${error}`,
      "reqServerErrors.txt"
    );
  }
};
...
const changePass = async (req, res) => {
  ...
  try {
    ...
    const [verifyAccess] = await connect_SQL.query(
      `SELECT EMAIL FROM company_password_resets WHERE TOKEN = ? `,
      [token]
    );
    if (verifyAccess[0].EMAIL) {
      await connect_SQL.query(
        "UPDATE company_users SET password = ? WHERE userlogin = ?",
        [hashedPwd, verifyAccess[0].EMAIL]
      );
      await connect_SQL.query(
        "DELETE FROM company_password_resets WHERE EMAIL = ?",
    ...
    logEvents(
      `resetPasswordController, changePass: ${error}`,
      "reqServerErrors.txt"
    );
  }
};
