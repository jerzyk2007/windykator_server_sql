...
const { LawPartner } = require("../config/roles_list");
...
const getDocumentsLawPartner =
  "SELECT NUMER_FV, KONTRAHENT, DATA_PRZEKAZANIA, NAZWA_KANCELARII FROM company_law_documents";
...
const getDataDocuments = async (id_user, info, profile) => {
  ...
  try {
    ...
    } else if (userType === "Kancelaria") {
      [filteredData] = await connect_SQL.query(`${getDocumentsLawPartner}`);
      return { data: filteredData };
    } else {
      return { data: [] };
    }
